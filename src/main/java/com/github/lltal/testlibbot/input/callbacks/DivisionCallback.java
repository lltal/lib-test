...
@CommandNames(value = Action.DIVISION, type = TypeCommand.CALLBACK)
@RequiredArgsConstructor
@Component
public class DivisionCallback {
    @NonNull
    ...
    public void execMultiply(
        ...
        SendMessage sendMessage = SendMessage.builder()
                .chatId(chatId)
                .text(data.getSecondNumber() == 0 ?
                        "на 0 делить нельзя" :
                        String.format("результат: %.2f", data.getFirstNumber() / data.getSecondNumber()))
                .build();
